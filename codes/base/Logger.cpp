...
Logger::Logger()
{
	...
	{
		/* 创建当前BufferLog */
		curBuf = std::make_shared<BufferLog>();
		/* 创建两块备用BufferLog */
		bufList.resize(2);
		(*bufList.begin()) = std::make_shared<BufferLog>();
		(*(++bufList.begin())) = std::make_shared<BufferLog>();
	}
}
Logger::~Logger()
...
std::shared_ptr<BufferLog>& Logger::useFul()
{
	...
	/* 不存在则创建一块新BufferLog并返回 */
	if (iter == bufList.end())
	{
		std::shared_ptr<BufferLog> p = std::make_shared<BufferLog>();
		/* 统一使用右值来提高效率 */
...
bool Logger::start(bool allowLog_)
{
	...
	else
	{
		...
		fd = open((string("./log/") + logFileName).data(), O_RDWR | O_APPEND | O_CREAT, 0644);
